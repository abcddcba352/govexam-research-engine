...
Tested subject adapters include ICC cricket news, FIDE chess news, PM India scheme-related announcements and the PM-KISAN description/exclusions. Existing PIB, RBI and ISRO collection is also scheduled. The Ministry of External Affairs press-release listing now uses its bounded server endpoint and then fetches the individual detail fragment, so international releases do not become navigation text. MoEFCC provides environment context and the Ministry of Culture provides a bounded history/culture reference. The NDMA extractor is covered by tests but disabled for cloud scheduling because its origin currently returns HTTP 522 from the Cloudflare edge. Sports extraction is limited to the connected sports; it is not comprehensive sports coverage. Announcements are research leads, not proof of current scheme eligibility or benefits. PM-KISAN has no reliably identified publication date and remains reference material requiring a version check.
...
NCERT, India Code and myScheme are listed with explicit adapter gaps. The Legislative Department Constitution page is also marked unavailable because its public page is client-rendered, the ASI homepage is too large and redirects across sections, and NDMA currently returns HTTP 522 from the Cloudflare edge; these need bounded article/PDF or alternate-host adapters before they can count as evidence. State-specific, specialist, mathematics and English evidence/generators are not made complete by this release. All their registered topics remain visible. No fabricated questions or automatic PASS audits fill these gaps.
