...
export class SupabaseMockRepository implements MockRepository {
  ...
  private mapRowToMock(data: any): MockTestRecord {
    const questions: MockQuestion[] = (data.questions || []).map((q: any) => ({
      ...
      bilingual: q.question_local ? {
        secondary_language: q.question_quality_audit?.secondary_language || 'Local language',
        question_text: q.question_local,
        options: [q.option_a_local, q.option_b_local, q.option_c_local, q.option_d_local],
        explanation: q.explanation_local || '',
        parity_score: Number(q.question_quality_audit?.bilingual_parity_score || 0),
      } : undefined,
      source_exam_date: q.question_quality_audit?.source_exam_date,
      content_lifecycle: q.question_quality_audit?.content_lifecycle,
      review_on: q.question_quality_audit?.review_on,
      expires_on: q.question_quality_audit?.expires_on,
      is_active: q.question_quality_audit?.is_active !== false,
    }));
    ...
    return {
      ...
      series_id: data.series_id,
      test_mode: data.test_mode,
      stage_id: data.generation_model_summary?.stage_id,
      paper_id: data.generation_model_summary?.paper_id,
      paper_title: data.generation_model_summary?.paper_title,
      preparation_mode: data.preparation_mode,
      preparation_basis_id: data.preparation_basis_id,
      created_at: data.created_at,
      duration_minutes: data.duration_minutes,
  ...
  async saveMock(mock: MockTestRecord): Promise<void> {
    ...
    const { error: mockErr } = await supabase.from('mocks').upsert({
      ...
      preparation_mode: mock.preparation_mode || 'PRE_NOTIFICATION_PREPARATION',
      test_mode: mock.test_mode || 'FULL_LENGTH',
      language: mock.sections.some(section => section.questions.some(question => question.bilingual)) ? 'en+local' : 'en',
      ...
      generation_model_summary: {
        stage_id: mock.stage_id || null,
        paper_id: mock.paper_id || null,
        paper_title: mock.paper_title || null,
        models: [...new Set(mock.sections.flatMap(section => section.questions.map(question => question.generation_model_id).filter(Boolean)))],
      },
    ...
    if (allQuestions.length > 0) {
      ...
      const qRows = allQuestions.map(q => ({
        ...
        question_local: q.bilingual?.question_text || null,
        option_a_local: q.bilingual?.options?.[0] || null,
        option_b_local: q.bilingual?.options?.[1] || null,
        option_c_local: q.bilingual?.options?.[2] || null,
        option_d_local: q.bilingual?.options?.[3] || null,
        correct_answer: ['A', 'B', 'C', 'D'][q.correct_option_index] || 'A',
        explanation_en: q.explanation,
        explanation_local: q.bilingual?.explanation || null,
        ...
        question_quality_audit: {
          ...((q as any).question_quality_audit || q.audit_result || {}),
          source_reference: q.source_reference,
          source_lineage: q.source_lineage,
          current_affairs_evidence: q.current_affairs_evidence,
          cognitive_level: q.cognitive_level,
          visual_specification: q.visual_specification,
          secondary_language: q.bilingual?.secondary_language,
          bilingual_parity_score: q.bilingual?.parity_score,
          source_exam_date: q.source_exam_date,
          content_lifecycle: q.content_lifecycle,
          review_on: q.review_on,
