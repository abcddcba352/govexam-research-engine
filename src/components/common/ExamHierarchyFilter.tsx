...
export const ExamHierarchyFilter: React.FC<ExamHierarchyFilterProps> = ({
  ...
  const activeExam = useMemo(() => {
    const matched = boardExams.find(e => e.title === selectedExamTitle) ||
      boardExams.find(e => e.exam_id === selectedExamId) ||
      boardExams[0] ||
      currentExam;
    return matched;
  }, [boardExams, selectedExamTitle, selectedExamId, currentExam]);
    ...
    <div className="bg-slate-50/80 border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-2xs space-y-3.5">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3 pb-3 border-b border-slate-200">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-semibold text-slate-600 bg-white p-2 rounded-xl border border-slate-200 shadow-2xs">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
            <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-900 font-bold border border-slate-200 max-w-[180px] truncate" title={activeExam?.title}>
              {activeExam?.title}
            </span>
            <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="space-y-1">
          ...
          >
            {boardExams.map(ex => (
              <option key={ex.exam_id} value={ex.title}>
                {ex.title}
              </option>
            ))}
          </select>
