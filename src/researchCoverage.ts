...
export const SUBJECTS = [
  {id:'sports',name:'Sports',terms:/sport|cricket|chess|olympic|athlet|tournament/i},
  ...
  {id:'economy',name:'Economy',terms:/econom|banking|budget|finance|financial/i},
...
export function subjectsForExam(exam:ExamRecord):SubjectId[] {
  ...
  const general=labels.some(t=>/^(general awareness|general knowledge|current affairs)(?:\s*\(|$)/i.test(t.trim()));
  return SUBJECTS.filter(s=>labels.some(t=>s.terms.test(t)) || (general && !['math','reasoning','english','specialist','state'].includes(s.id))).map(s=>s.id);
}
...
export function buildCoverage(exam:ExamRecord,sources:SourceRecord[],cutoff:string) {
  ...
  const topics=[...new Set([...exam.pattern.sections,...exam.syllabus_topics])].map(topic=>{
    const broad=/^(general awareness|general knowledge|current affairs)(?:\s*\(|$)/i.test(topic.trim());
